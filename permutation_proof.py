...
class ExplainingPermutations(Scene):
	...
	def circle_permute(self, dots, labels, permutation: dict):
		transforms = []
		fadein_arrows = []
		fadeout_arrows = []
		fade_out_labels = [FadeOut(label) for label in labels]
		...
		for key, value in permutation.items():
			...
			transforms.append(Transform(this, other))
			arrow = make_arrow_between(this, other)
			fadein_arrows.append(FadeIn(arrow))
			fadeout_arrows.append(FadeOut(arrow))

		self.play(*fadein_arrows, *fade_out_labels)
		self.play(*transforms, run_time=3)
		self.num_permutes += 1
		...
		self.play(*fadeout_arrows, *fade_in_labels)
	...
	def permute(self, dots, permutation: dict):
		...
		for key, value in permutation.items():
			...
			arc = ArcBetweenPoints(this.get_center(), other.get_center(), radius=halfdist, stroke_opacity=0.5)
	...
	def construct(self):
		...
		self.play(
			ApplyMethod(group_of_dots.shift, UP * 2),
		...
		new_dot4 = Dot(circle.point_at_angle(TAU / 4 - TAU / 5))
		new_dot5 = Dot(circle.point_at_angle(TAU / 4 + 3 * TAU / 5))
		...
		self.wait()
		self.circle_permute(dots, labels, permutation)
		self.wait()
		self.circle_permute(dots, labels, permutation)
		self.wait()
		self.circle_permute(dots, labels, permutation)
...
class BreakingIntoCycles(ZoomedScene, MovingCameraScene):
	...
	def construct(self):
		...
		cycle1name = Text('Cycle 1').move_to(10 * LEFT + 4 * DOWN).scale(0.5)
		cycle2name = Text('Cycle 2').move_to(7 / 3 * LEFT + 4 * DOWN).scale(0.5)
		cyclemname = Text('Cycle m').move_to(10 * RIGHT + 4 * DOWN).scale(0.5)
...
class BringingInto2Circles(ZoomedScene, MovingCameraScene):
	def permute(self, dots, labels, permutations, *cycles):
		...
		self.play(*transforms, *fade_out_arrows, run_time=3)
